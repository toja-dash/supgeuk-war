...
def generate_market_brief(df: pd.DataFrame) -> dict:
    ...
    if m_i_total > 1.0 and m_f_total > 1.0:
        brief = "오늘 시장은 **동반 매집 구간(Type B)**에 진입하며 강한 상승 동력이 형성됐습니다."
        ...
        brief = "오늘 시장은 **동반 분산 매도(Type A)** 압력으로 인해 전반적인 약세를 면치 못했습니다."
    elif m_f_total < -1.0 and m_i_total >= -1.0:
    ...
    # 3단 위험
    if count_type_a >= 30:
        brief += f" ⚠️ 동반 분산 매도(Type A) 종목이 **{count_type_a}개**로 집중 발생, 평단가 방어선 이탈에 유의해야 합니다."
