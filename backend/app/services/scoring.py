...
def generate_insights(row: pd.Series) -> dict:
    ...
    elif t == 'C': signal_card_text = "외인 매수 vs 기관 매도 충돌"
    elif t == 'D': signal_card_text = "기관 매수 vs 외인 매도 충돌"
    
    headline = "⚪ 뚜렷한 수급 신호가 관찰되지 않습니다."
    if t == 'A': headline = "🔴 큰손 동반 매도 — 평단가 방어선이 무너졌습니다."
    elif t == 'B': headline = "🟢 기관·외인 쌍끌이 매수 — 가장 신뢰도 높은 매수 신호입니다."
    elif t == 'C': headline = "🟡 외인은 사고 기관은 팝니다 — 수급 해석이 갈리는 구간입니다."
    elif t == 'D': headline = "🔵 기관 단독 방어 매수 — 외인 매도 압력에도 기관이 받치고 있습니다."
    
    q_label = ""
    if quad == 'BOTH_BUY': q_label = "쌍끌이 매수(Q1)"
    elif quad == 'INST_DEFENSE': q_label = "기관 방어(Q2)"
    elif quad == 'BOTH_SELL': q_label = "쌍끌이 매도(Q3)"
    elif quad == 'FRGN_LEAD': q_label = "외인 주도(Q4)"
