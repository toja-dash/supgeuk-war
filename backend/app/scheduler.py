...
async def job_intra_snapshot():
    ...
    t_date = latest_trading_day(datetime.now(KST))
...
def setup_scheduler():
    ...
    scheduler.add_job(job_master_sync, CronTrigger(hour=6, minute=0, timezone=KST), id="master_sync")
    
    # intra_snapshot: 장중 09:30, 10:30, 11:30, 13:30, 14:30
    scheduler.add_job(job_intra_snapshot, CronTrigger(hour="9,10,11,13,14", minute=30, timezone=KST), id="intra_snapshot")
    
    # eod_provisional: 15:30
    scheduler.add_job(job_eod_provisional, CronTrigger(hour=15, minute=30, timezone=KST), id="eod_provisional")
    
    # eod_confirmed: 18:00
    scheduler.add_job(job_eod_confirmed, CronTrigger(hour=18, minute=0, timezone=KST), id="eod_confirmed")
