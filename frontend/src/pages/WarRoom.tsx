...
export default function WarRoom() {
  ...
  const { data: brief, isLoading: isBriefLoading, isError: isBriefError } = useQuery({
  ...
  const briefText = isBriefError
    ? '시장 데이터를 불러오지 못했습니다.'
    : isBriefLoading
      ? '데이터 로딩 중...'
      : brief?.market_brief_text || '표시할 시장 데이터가 없습니다.';
    ...
    <div className="flex flex-col gap-6">
      <section>
        ...
        <div className="mb-4 rounded-lg border border-border-subtle bg-surface-2 p-4">
          <p className="text-base text-ink-primary">{briefText}</p>
        </div>
      ...
      <div className="mt-8 pb-4 text-xs text-ink-muted">
        * 본 서비스의 수급 데이터 분석 결과는 투자 참고용이며 실제 투자 결과에 대한 책임을 지지 않습니다.
      </div>
    </div>
  );
}
