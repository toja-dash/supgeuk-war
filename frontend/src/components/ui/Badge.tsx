...
export function DefenseBadge({ state }: { state: keyof typeof DEFENSE_CLASSES | string | null }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 rounded-full bg-surface-2 px-3 py-1 text-xs font-medium">
...
export function ChangePct({ value }: { value: number | null }) {
  if (value === null || value === undefined) {
    return <span className="font-numeric text-num-flat">-</span>;
  }
